...
-- ================================================================
-- 1. LIMPAR/ATUALIZAR DADOS ANTIGOS
-- ================================================================

-- Opção A: Limpar TODOS os dados antigos (DESCOMENTE se quiser limpar tudo)
-- DELETE FROM police_stations;

-- Opção B: Atualizar tipos antigos para os novos padrões (RECOMENDADO)
-- Converter tipos antigos incompatíveis para DEAM
UPDATE police_stations 
SET type = 'DEAM' 
WHERE type NOT IN (
  'DEAM', 'Delegacia', 'CDCM', 'Centro de Referência', 
  'Casa da Mulher Brasileira', 'Centro de Atendimento Integrado',
  'Casa Abrigo', 'Hospital', 'ONG'
);

-- ================================================================
-- 2. EXPANDIR TIPOS PERMITIDOS
-- ================================================================

-- Remover constraint antigo
ALTER TABLE police_stations DROP CONSTRAINT IF EXISTS police_stations_type_check;

-- Adicionar novo constraint com todos os tipos
ALTER TABLE police_stations ADD CONSTRAINT police_stations_type_check 
CHECK (type IN (
  ...
  'Casa Abrigo',                       -- Casa de Acolhimento (futuro)
  'Hospital',                          -- Hospital de Referência (futuro)
...
CREATE INDEX IF NOT EXISTS idx_police_stations_location ON police_stations USING gist(
  ll_to_earth(latitude, longitude)
);

COMMENT ON COLUMN police_stations.type IS 'Tipos: DEAM (Delegacia), CDCM (Centro de Defesa), Centro de Referência, Casa Abrigo';
